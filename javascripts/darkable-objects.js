...
(function () {
  ...
  const OBJECT_SELECTOR =
    'object[type="image/svg+xml"][data*="#darkable"]';
  ...
  function isDarkMode() {
    ...
    return (
      (host && host.getAttribute("data-md-color-scheme")) ===
      DARK_SCHEME
    );
  }
  ...
  function fixStartEndStateColors(svg) {
    ...
    function areNested(bb1, bb2, threshold = 0.7) {
      ...
      const center1 = {
        x: (bb1.minX + bb1.maxX) / 2,
        y: (bb1.minY + bb1.maxY) / 2,
      };
      const center2 = {
        x: (bb2.minX + bb2.maxX) / 2,
        y: (bb2.minY + bb2.maxY) / 2,
      };
    ...
    const candidatePolygons = Array.from(
      ...
      const hasDarkStroke =
        style.includes("stroke:#222222") ||
        style.includes("stroke:#222");
    ...
    candidatePolygons.forEach((polygon) => {
      ...
      if (nestedPolygon) {
        ...
        const isOuter =
          bb1.width > bb2.width || bb1.height > bb2.height;
  ...
  function renderDiagram(diagram) {
    ...
    if (svg) {
      diagram.naturalWidth =
        getNaturalWidth(svg) || diagram.naturalWidth;
  ...
  async function inlineDiagram(obj) {
    ...
    try {
      // Fetch content BEFORE replacing the object to avoid layout jumps/flashing
      ...
      // Populate the container with initial content
      renderDiagram(diagram);

      // Now replace the object with the ready container
  ...
  async function transformObjects(root = document) {
    const objects = Array.from(
      root.querySelectorAll(OBJECT_SELECTOR)
    );
    diagrams.clear();
